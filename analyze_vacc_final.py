...
pro_vaccine_hashtags = ['vaccinessavelives', 'getyourshot', 'vaccineforall', 'getvaccinated', 'vaccineswork']
anti_vaccine_hashtags = ['vaccinescauseautism', 'antivaccine', 'antivax', 'CoronaHoax']
...
def get_probabilities(community2tweets):
    ...
    for c,tweet_list in community2tweets.items():
        for user_tweets in tweet_list:
            ok_black = is_tag_in_text(pro_vaccine_hashtags, user_tweets)
            ok_blue = is_tag_in_text(anti_vaccine_hashtags, user_tweets)
...
def plot_black_blue(d_probs, out_file):
    ...
    df = pd.DataFrame(data, columns=['community', 'pro-vaccine', 'anti-vaccine'])
    # df = df.set_index('community')
    df = df.sort_values(by=['anti-vaccine'])
    ...
    color_blue = '#0000FF'
    index = df.index
    column0 = df['pro-vaccine']
    column1 = df['anti-vaccine']
    title0 = 'pro-vaccine'
    title1 = 'anti-vaccine'
...
def plot_stats(d_stats, network_type):
    for k in d_stats.keys():
        ...
        plt.title(k)
        plt.savefig('echo_chambers/stats-'+network_type+'_'+k+'_vacc_final.png')
        plt.clf()
...
def get_distance(g, nodes, influencers):
    ...
    for node in nodes:
        for inf in influencers:
            if node != inf:
                distance = g.distances(node, inf)[0][0]
...
def plot_shortest_distance_to_influencers(g, nodes1, nodes2, influencers1, influencers2):
    ...
    fig, ax = plt.subplots()
    plt.xticks([0, 1], labels=['community15', 'community16'])
    bar_plot(ax, data, total_width=.8, single_width=.9)
    plt.savefig('echo_chambers/vacc_final-quoted_controversy.png')
    plt.clf()
...
def plot_boxplot(data):
    ...
    # show plot
    plt.savefig('echo_chambers/vacc_final-quoted_controversy_rawc.png')
...
def main():
    if len(sys.argv) != 2:
        ...
        out_file = 'echo_chambers/plot-vacc_final_'+network_type+'.png'
